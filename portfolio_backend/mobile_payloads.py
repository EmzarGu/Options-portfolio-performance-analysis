...
from portfolio_backend.cycle_projection import (
    build_active_cycle_projection,
    build_future_cycle_projections,
    build_monthly_target,
    current_calendar_month_row as _current_month_row,
    month_end as _month_end,
    monthly_projection_values as _monthly_projection_values,
    monthly_row_for_projection as _monthly_row_for_projection,
    monthly_target_status as _monthly_target_status,
)
...
def _mobile_month_row(
    ...
    *,
    ...
    if state is not None:
        ...
        out["open_premium_collected"] = json_safe(projection["open_premium_collected"])
        ...
        if active_month is not None and active_month.to_period("M") == month_end.to_period("M"):
            ...
            if active_cycle.get("projected_return_ropc") is not None:
                out["projected_return_ropc"] = json_safe(active_cycle.get("projected_return_ropc"))
...
def build_current_month_performance(
    ...
    *,
    ...
    return {
        ...
        "open_premium_collected": month_row["open_premium_collected"],
...
def build_open_option_short_rows(state, *, sort: str = "moneyness_risk", limit: Optional[int] = None) -> List[Dict[str, Any]]:
    ...
    for source_index, row in enriched.iterrows():
        ...
        projected_pnl = None
        if not missing_price and accounting_open_premium is not None and strike is not None and quantity is not None:
            intrinsic_per_share = (
                max(current_price - strike, 0.0)
                if option_type.lower() == "call"
                else max(strike - current_price, 0.0)
            )
            projected_pnl = accounting_open_premium - intrinsic_per_share * abs(quantity) * CONTRACT_MULTIPLIER
        ...
        rows.append(
            {
                ...
                "projected_pnl": json_safe(projected_pnl),
