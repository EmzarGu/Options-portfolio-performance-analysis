...
| ROLL-CALL-001 | Same-order call roll | If close and replacement share IBKR execution group, realize the old lot on close and keep the replacement premium open until the replacement closes, expires, or is assigned. | `test_ibkr_pipeline_keeps_same_day_roll_replacement_premium_open_until_close`; `test_ibkr_pipeline_keeps_same_day_roll_replacement_open_with_actual_unrealized_premium` | Covered |
...
| ROLL-PUT-001 | Same-order put roll | Same realized/open split as calls, with no stock effect unless later assigned. | `test_ibkr_pipeline_keeps_same_order_put_roll_replacement_open_until_expiration` | Covered |
